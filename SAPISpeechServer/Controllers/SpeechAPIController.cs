...
/// <summary>
///     REST endpoints for the Speech API.
/// </summary>
[ApiController]
[Route("api/speech")]
public class SpeechApiController : ControllerBase
{
    ...
    public IEnumerable<VoiceDetails> GetVoices()
    {
        ...
        for (var i = 0; i < voices.Count; i++)
        {
            ...
            try
            {
                var name = token.GetAttribute("Name");
                var vendor = token.GetAttribute("Vendor");
                var language = GetLanguageName(token.GetAttribute("Language"));
                var gender = token.GetAttribute("Gender");
    ...
    /// <summary>
    ///     Speaks the given text and returns the audio as a WAV file.
    /// </summary>
    /// <param name="text">The text to speak.</param>
    /// <param name="vendor">The vendor of the voice to use (optional)</param>
    /// <param name="name">The name of the voice to use (optional)</param>
    /// <param name="lang">The language of the voice to use, such as en-US or it-IT (optional)</param>
